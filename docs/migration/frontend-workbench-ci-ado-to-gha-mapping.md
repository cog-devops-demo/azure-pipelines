...
| `trigger.paths.include: [services/frontend-workbench/**]` | `on.push.paths: ['services/frontend-workbench/**']` | identical scope; the workflow file itself is deliberately not in the filter (see G9) |
...
| G9 | The workflow file is not in the trigger paths (same as ADO). Adding it would make every workflow-only change fail at npm ci until the app source exists (G6); once services/frontend-workbench has a package.json, add '.github/workflows/frontend-workbench-ci.yml' to both push.paths and pull_request.paths. | Deferred; follow-up |
