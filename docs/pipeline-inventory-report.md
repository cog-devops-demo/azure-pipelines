...
| 111 | scenario-runner-weekly | `services/scenario-runner/azure-pipelines.yml` | **unknown** | Python (Monte Carlo) | Schedule `0 22 * * 5` + manual with parameters | **None** — inline | Non-build workload | R1 no owner; R14 `high-memory-pool` (128 GB / 32 cores) self-hosted; R15 configured timeout 360 min sits exactly at GHA's 6 h hosted job limit, avg 228 min; R17 CI used as compute | **Leave on ADO / wrong platform** — needs dedicated compute (AWS Batch / K8s / ACI). |
| 112 | var-scenario-sweep-nightly | `night-jobs/compute/var-scenario-sweep.yml` | team-quant | Python (VaR sweep, 50 000 scenarios) | Schedule `0 1 * * 1-5` | **None** — inline | Non-build workload | R14 `high-memory-pool`; R15 240 min timeout, avg 172 min; R12 `pipeline.startTime` expression has no GHA equivalent; R6 data-warehouse credentials | **Leave on ADO / wrong platform** — dedicated compute platform. |
...
platform. Runtimes leave no headroom: 111 is configured with a 360 min timeout, exactly
GHA's hosted job ceiling, and averages 228 min, so any growth in scenario count breaches
it. All four depend on the
